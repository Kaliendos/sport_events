...
from src.event.shemas.event_schemas import (
    ReadEvent, CreateEvent, ReadEventItem, UpdateEvent
)
...
@router.get("/{event_id}")
async def get_event(
        event_id: int, service: EventService = Depends()
) -> ReadEventItem:
...
async def going_on_event_create(obj_id: int, user=Depends(current_user)):
    async with async_session_maker() as session:
        event = await session.scalar(select(Event).where(Event.id == obj_id))
        await session.execute(
            insert(going_table).values(event_id=event.id, user_id=user.id)
        )
        await session.commit()


@router.delete("/going/{obj_id}")
async def going_on_event_delete(obj_id: int, user=Depends(current_user)):
    async with async_session_maker() as session:
        await session.execute(
            delete(going_table).filter(
