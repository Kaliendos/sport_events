...
Revision ID: 60254cb218a9
Revises: 722d97bab966
Create Date: 2023-07-27 17:52:21.301310
...
revision = '60254cb218a9'
down_revision = '722d97bab966'
branch_labels = None
depends_on = None


def upgrade() -> None:
    op.add_column('event', sa.Column('location', geoalchemy2.types.Geometry(geometry_type='POINT', srid=4326, from_text='ST_GeomFromEWKT', name='geometry'), nullable=True))
    op.drop_index('idx_event_geometry', table_name='event')
...
def downgrade() -> None:
    ...
    op.add_column('event', sa.Column('geometry', geoalchemy2.types.Geometry(geometry_type='POINT', srid=4326, from_text='ST_GeomFromEWKT', name='geometry', _spatial_index_reflected=True), autoincrement=False, nullable=True))
    op.drop_index('idx_event_location', table_name='event', postgresql_using='gist')
    op.create_index('idx_event_geometry', 'event', ['geometry'], unique=False)
